...
let renderLookBackSegment = async (documentRef, graphDOMRef, insightDOMRef, dataObj, dataURI, firstLoc, secondLoc) => {
  ...
  drawLookBackScatterChart(dataObj.dataRaw, dataObj.dataAvg, `lookBack_${firstLoc}To${secondLoc}`,
    `${locationA} -> ${locationB} Scatter`, insightObj["trip-lowest-time-val"].value-5, calculatedCeil+5);
}